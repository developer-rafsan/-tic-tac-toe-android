const CLOUD_RELAY_URL = 'wss://tic-tac-toe-relay.your-subdomain.workers.dev/ws';
...
export const findServer = async (onProgress) => {
  if (onProgress) onProgress('Connecting to relay server...');
  const relay = await tryConnect(CLOUD_RELAY_URL, 5000);
  if (relay) {
    if (onProgress) onProgress('Connected to relay');
    return relay;
  }
